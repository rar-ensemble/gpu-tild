...
poly_types = 3              # Number of different polymer types
rho0 = 0.075                  # density of the polymer
...
phi = np.array([0.5,0.25,0.25])       # Phi for each poly_type
q = [[-1, 1],[1.],[-1.]]               # Charge of monomers on each block
N = [[Na, Nb],[1],[1]]      # Must be a list of lists. Secondary lists are lengths of each block for that polymer type
...
box = np.array([25., 25., 125])                 # Size of the simulation box

Rg = ((np.sum(N[0])-1.)/6.)**0.5      # Radius of Gyration
...
if ( tot_charge != 0.0 ):                  # Check to make sure
